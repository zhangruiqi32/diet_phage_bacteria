##Script to Figure 6-A and Supplementary fig9-D.
...
############################## [figS9-D] Processing and Painting #####################################
x <- Diet1
i<- prj_index
x <- x[rowSums(is.na(x)==F)>0,]
x[is.na(x)] <- 0
...
# NR is classified as Others(Nutrition)
df$Nutrition[df$diet=="NR"] <- "Others"
...
# NR is classified as Others(Nutrition)
spe_x_prj_diets_tongji3$Nutrition[spe_x_prj_diets_tongji3$diet=="NR"] <- "Others"
